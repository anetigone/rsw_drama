...
async function convertWordToHtml(inputPath, activityId) {
  try {
    ...
    // 创建活动专用的图片文件夹
    const activityImageDir = path.join(config.outputDir, `activity_${activityId}`);
    if (!fs.existsSync(activityImageDir)) {
      fs.mkdirSync(activityImageDir, { recursive: true });
      console.log(`创建活动图片文件夹: activity_${activityId}`);
    }
    ...
    const result = await mammoth.convertToHtml({
      ...
      convertImage: mammoth.images.imgElement((image) => {
        return image.read().then((imageBuffer) => {
          ...
          // 返回HTML img标签，使用绝对路径
          return {
            src: `/images/activities/activity_${activityId}/${imageName}`
          };
        });
      })
    });
